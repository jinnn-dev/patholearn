...
from app.utils.slide_utils import (
...
def create_slide(
    *,
    collection: Collection = Depends(get_slide_collection),
    ...
    if crud_slide.slide_with_name_exists(collection=collection, name=name):
        raise HTTPException(
            status_code=400, detail="Slide with this name already exists"
        )
    ...
    return Slide(slide_id=file_id, name=name, status=SlideStatus.RUNNING)
...
def read_slides(
    *,
    collection: Collection = Depends(get_slide_collection),
    slideid: List[str] = Query(None),
    status: SlideStatus = Query(None),
    metadata: bool = Query(True),
) -> List[Slide]:
    if slideid:
        ...
        slides = crud_slide.get_all_slides(
            collection=collection, status=status, with_metadata=metadata
        )
    if metadata:
...
def get_slide_by_id(
    *,
    ...
    slide = crud_slide.get_slide(
        collection=collection, slide_id=slide_id, with_metadata=metadata
    )
    if metadata:
...
def get_slide(*, collection: Collection = Depends(get_slide_collection), slide_id: str):
    slide = crud_slide.get_slide(
        collection=collection, slide_id=slide_id, with_metadata=False
    )

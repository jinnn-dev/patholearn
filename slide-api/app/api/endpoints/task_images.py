...
@router.get("")
def get_task_images_by_id(
    *,
    collection: Collection = Depends(get_task_image_collection),
    taskimageid: List[str] = Query(None),
) -> List[TaskImage]:
    if taskimageid:
        task_images = crud_task_image.get_multi_by_ids(
            collection=collection, ids=taskimageid, filter_query={"_id": False}
        )
    else:
        task_images = crud_task_image.get_multi(
...
def create_task_image(
    *,
    ...
    for index, image in enumerate(images):
        ...
        try:
            task_image_object_id = crud_task_image.create(
                collection=collection,
                obj_in=CreateTaskImage(task_image_id=task_image_id, name=names[index]),
            )
            app.minio_wrapper.create_task_image(
                file_name=task_image_file_name, file_content=task_image_file_name
            )
            results.append(
                crud_task_image.get_by_objectId(
                    collection=collection, object_id=task_image_object_id
                )
            )
...
def update_task_image(
    *,
    ...
    for update_schema in update_schemas:
        ...
        crud_task_image.update(
            collection, obj_in=update_schema, entity_id_value=task_image_id
        )

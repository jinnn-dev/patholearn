...
def convert_slide(source_file_with_extension: str):
    ...
    try:
        ...
        if file_extension.lower() == ".dcm":
            ...
            thumbnail_path = (
                f"{path_to_slide_folder}/{thumbnail_uuid}/{thumbnail_uuid}.jpeg"
            )
        ...
        minio_client.delete_folder(
            bucket_name=MinioWrapper.pyramid_bucket, folder_path=f"{slide_uuid}"
        )
        for connection in get_slide_collection():
            crud_slide.crud_slide.update(
                connection,
                entity_id_value=slide_uuid,

...
from app.utils.slide_utils import get_file_name_and_file_extension, openslide_can_load


class SlideConverter:
    @staticmethod
    def save_thumbnail(
        file_path: str, file_name_with_extension: str, minio_client: MinioClient
    ):
        """
        ...
        """
        file_id, file_extension = get_file_name_and_file_extension(
        ...
        if openslide_can_load(file_extension):
            try:
                thumbnail = pyvips.Image.openslideload(
                    file_path, associated="thumbnail"
                )
            except:
        ...
        thumbnail_path = rf"{Config.TEMP_IMAGES_FOLDER}/{file_id}/thumbnail.jpeg"
    ...
    def save_deep_zoom(
        ...
        pyramid_images_to_save = [
            image_path
            for image_path in glob.glob(
                rf"{dzi_folder_path}/dzi_files/**/*.jpeg", recursive=True
            )
        ]
        ...
        minio_client.create_object(
            bucket_name=MinioWrapper.pyramid_bucket,
            file_name=rf"{dzi_minio_name}/dzi.dzi",
            file_content=rf"{dzi_folder_path}/dzi.dzi",

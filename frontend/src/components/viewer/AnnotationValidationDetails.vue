...
<template>
  <BlurredContainer class='fixed left-2 bottom-2 w-72 max-h-72 z-20 rounded-lg overflow-hidden'>
    ...
    <div class='absolute top-0 w-full h-full bg-gray-700/80 backdrop-blur-sm flex justify-center items-center'
         v-if='loading'>
      <spinner></spinner>
    </div>
    <div class='max-h-60 overflow-y-auto px-2 flex flex-col gap-2 py-2'>
      <div v-for='result in validationResult'
           :key='result.id'

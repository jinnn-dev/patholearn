<template>
  <div class="transition flex items-center my-2 mx-2 p-2 bg-gray-500 rounded-xl hover:bg-gray-400" :title="question">
    <div v-if="!isOwner">
      <Icon name="circle" v-if="!userSolution || userSolution?.solution_data?.length === 0 || !userSolution.task_result" class="text-gray-200"></Icon>
      <Icon name="check-circle" v-else-if="userSolution.task_result.task_status === TaskStatus.CORRECT" class="text-green-400"></Icon>

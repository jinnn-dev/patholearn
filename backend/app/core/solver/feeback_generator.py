...
class FeedbackGenerator:
    ...
    @staticmethod
    def generate_point_feedback(*, solve_result: Tuple[Dict[str, List[PointResult]], List[str]],
                                task_result: TaskFeedback, min_correct: int, knowledge_level: int,
                                check_name: bool = False, ) -> TaskFeedback:
        """
        ...
        matched_ids = FeedbackGenerator.__filter_best_match(solve_result[0], key="distance")
                ...
                if len(matched_ids[key]) > 1:
                    ...
                    if check_name:
                        ...
                        if not annotation.name_matches:
                            ...
                            TaskResultFactory.append_result_detail(annotation_id=annotation.id,
                                                                   status=status,
                                                                   percentage=1.0, task_result=task_result)
                        else:
                            correct_count += 1
                            TaskResultFactory.append_result_detail(annotation_id=annotation.id,
    ...
    @staticmethod
    def generate_line_feedback(*, solve_result: Tuple[Dict[str, List[LineResult]], List[str]],
                               task_result: TaskFeedback,
                               min_correct: int, knowledge_level: int, check_name: bool = False) -> TaskFeedback:
        """
        ...
        matched_ids = FeedbackGenerator.__filter_best_match(solve_result[0], key="percentage_outside")
        no_match_ids = solve_result[1]
        ...
        for key in matched_ids:
            for annotation in matched_ids[key]:
                ...
                if len(matched_ids[key]) > 1:
                    ...
                    task_result_detail = FeedbackGenerator.generate_detail_line_result(annotation, check_name,
                                                                                       knowledge_level)
        ...
        if not task_result.task_status:
            if correct_count < min_correct:
                ...
                task_result.response_text = "Einige Annotationen sind noch nicht ganz richtig!"
            else:
                TaskResultFactory.correct_status(task_result)
    ...
    @staticmethod
    def generate_polygon_feedback(*, solve_result: Tuple[Dict[str, List[PolygonResult]], List[str], List[str]],
                                  task_result: TaskFeedback, min_correct: int, knowledge_level: int,
                                  check_name: bool = False) -> TaskFeedback:
        """
    ...
    @staticmethod
    def generate_detail_line_result(line_result: LineResult, check_name: bool,
                                    knowledge_level: int) -> AnnotationFeedback:
        """
        ...
        length_border = FeedbackConfig.get_weighted_border(
            border=FeedbackConfig.PERCENTAGE_LENGTH_DIFFERENCE, knowledge_level=knowledge_level, decent_value=0.2)
    ...
    @staticmethod
    def generate_detail_polygon_result(polygon_result: PolygonResult, check_name: bool,
                                       knowledge_level: int) -> AnnotationFeedback:
        """
                ...
                if "name_matches" in annotation:
                    if not annotation.name_matches:
                        task_result.result_detail.append(
                            AnnotationFeedback(id=annotation.id, status=TaskStatus.WRONG_NAME))
    ...
    def __filter_best_match(matched_ids: Dict[str, List[Union[PointResult, LineResult, PolygonResult]]], *, key: str) -> \
        ...
        for solution_id in matched_ids:
            for idx, item in enumerate(matched_ids[solution_id]):
                duplicate_items = FeedbackGenerator.__has_different_hit(matched_ids, solution_id, item.id)
    ...
    @staticmethod
    def __generate_no_match_feedback(task_result: TaskFeedback, no_match_ids: List[str]) -> List[AnnotationFeedback]:
        """
        ...
        for no__match_id in no_match_ids:
            TaskResultFactory.append_result_detail(annotation_id=no__match_id, status=TaskStatus.WRONG, percentage=0.0,
                                                   task_result=task_result)
        return detail_results

    @staticmethod
    def __generate_invalid_feedback(task_result: TaskFeedback, invalid_ids: List[str]) -> List[AnnotationFeedback]:
        """
        ...
        for no__match_id in invalid_ids:
            TaskResultFactory.append_result_detail(annotation_id=no__match_id, status=TaskStatus.INVALID,
                                                   percentage=0.0,
                                                   task_result=task_result)
        return detail_results

    @staticmethod
    def __has_different_hit(matched_ids: Dict[str, List[Union[PointResult, LineResult, PolygonResult]]],
                            solution_id: str, result_id: str) -> List[Tuple[
        Union[str, None], Union[PointResult, LineResult, PolygonResult, None]]]:
        """

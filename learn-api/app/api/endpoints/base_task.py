...
from app.schemas.base_task import (
    BaseTask,
    BaseTaskCreate,
    BaseTaskDetail,
    BaseTaskUpdate,
)
from app.schemas.membersolution_summary import (
...
from app.schemas.task import TaskAnnotationType, TaskCreate, TaskStatus, TaskType
...
def create_base_task(
    *,
    ...
    base_task_in: BaseTaskCreate,
    current_user: User = Depends(get_current_active_superuser),
) -> Any:
    duplicate_base_task = crud_base_task.get_by_name(
        db, name=base_task_in.name, task_group_id=base_task_in.task_group_id
    )

    check_if_user_can_access_course(
        db, user_id=current_user.id, course_id=base_task_in.course_id
    )

    if duplicate_base_task:
        raise HTTPException(status_code=400, detail="BaseTask name already exists")
...
def create_base_task_from_csv(
    *,
    db: Session = Depends(get_db),
    base_task_in: str = Form(...),
    csv_file: UploadFile = File(...),
    image_dicts: str = Form(...),
) -> Any:
    ...
    try:
        tab = pd.read_csv(
            StringIO(str(csv_file.file.read(), "utf-8")), nrows=1, sep="\t"
        ).shape[1]
            ...
            if index == curr_divider or i == len(df.index) - 1:
                ...
                for name in image_names:
                    ...
                    if len(path) > 0:
                        ...
                        label_update_dict.append(
                            {
                                "task_image_id": path[0]["task_image_id"],
                                "label": sub_df[sub_df["name"] == name]["class"].values[
                                    0
                                ],
                            }
                        )
        ...
        response = requests.put(
            settings.SLIDE_URL + "/task-images", json=label_update_dict
        )
        return base_task
        ...
        raise HTTPException(status_code=500, detail="BaseTask could not be created")


@router.put("", response_model=BaseTask)
def update_base_task(
    *,
    db: Session = Depends(get_db),
    current_user: User = Depends(get_current_active_superuser),
    obj_in: BaseTaskUpdate,
) -> BaseTask:
    ...
    duplicate_base_task = crud_base_task.get_by_name(
        db, name=obj_in.name, task_group_id=base_task.task_group_id
    )
...
def read_task_details(
    *,
    ...
    if crud_course.is_not_member_and_owner(
        db, course_id=base_task.course_id, user_id=current_user.id
    ):
...
def get_base_tasks_with_no_group(
    *,
    db: Session = Depends(get_db),
    course_id: int,
    current_user: User = Depends(get_current_active_user),
...
def get_membersolution_summary(
    *,
    db: Session = Depends(get_db),
    short_name: str,
    current_user: User = Depends(get_current_active_superuser),
    ...
    for member in members:
        ...
        for task in base_task.tasks:
            user_solution = crud_user_solution.get_solution_to_task_and_user(
                db, user_id=member.id, task_id=task.id
            )
            if user_solution:
...
def save_user_solution(
    *,
    db: Session = Depends(get_db),
    user_solution_in: UserSolutionCreate,
    current_user: User = Depends(get_current_active_user),
) -> Any:
    user_solution = crud_user_solution.get_solution_to_task_and_user(
        db, user_id=current_user.id, task_id=user_solution_in.task_id
    )
    ...
    if user_solution is not None:
        ...
        crud_user_solution.update(
            ...
            obj_in=UserSolutionUpdate(**user_solution_in.dict()),
        )
...
def remove_user_solution(
    *,
    db: Session = Depends(get_db),
    task_id: int,
    current_user: User = Depends(get_current_active_user),
) -> UserSolution:
    return crud_user_solution.remove_by_user_id_and_task_id(
        db=db, user_id=current_user.id, task_id=task_id
    )


@router.put("/{task_id}/userSolution", response_model=UserSolution)
def update_user_solution(
    *,
    db: Session = Depends(get_db),
    task_id: int,
    user_solution_in: UserSolutionUpdate,
    current_user: User = Depends(get_current_active_user),
) -> Any:
    user_solution_in.user_id = current_user.id
    user_solution_in.solution_data = json.loads(user_solution_in.solution_data)
    ...
    #     return item
    db_obj = crud_user_solution.get_solution_to_task_and_user(
...
def get_task_hints(
    *,
    ...
    hints = crud_task_hint.get_hints_by_task(
        db, task_id=task_id, mistakes=failed_attempts
    )
    return hints
...
def get_statistic_to_base_task(*, db: Session = Depends(get_db), short_name: str):
    ...
    (
        task_statistics,
        task_ids,
    ) = crud_task_statistic.get_oldest_task_statistics_to_base_task_id(
        db, base_task_id=base_task.id
    )
                ...
                if not image_uuid in task.solution:
                    image = next(
                        (
                            ...
                            if image["task_image_id"] == image_uuid
                        ),
                        None,
                    )
                    if image:
    ...
    most_wrong_picked_images = dict(
        sorted(most_wrong_picked_images.items(), key=lambda elem: elem[1], reverse=True)
    )
    most_wrong_picked_images_sorted = {
        key: most_wrong_picked_images[key]
        for key in list(most_wrong_picked_images)[0:5]
    }
    ...
    if len(most_wrong_picked_images_sorted.keys()) > 0:
        for key in most_wrong_picked_images_sorted.keys():
            image = next(
                (image for image in loaded_images if image["task_image_id"] == key),
                None,
            )
            if image:
                result_images.append(image)
                image["amount"] = most_wrong_picked_images_sorted[
                    image["task_image_id"]
                ]
    ...
    for item in most_wrong_classified_images:
        most_wrong_classified_images[item] = dict(
            sorted(
                most_wrong_classified_images[item].items(),
    ...
    most_wrong_classified_images = dict(
        sorted(
            most_wrong_classified_images.items(),
            key=lambda elem: list(elem[1].values())[0],
            reverse=True,
        )
    )

    most_wrong_classified_images = {
        key: most_wrong_classified_images[key]
        for key in list(most_wrong_classified_images)[0:5]
    }
    ...
    for key, value in most_wrong_classified_images.items():
        ...
        for detail_key, detail_value in value.items():
            detail.append(
                WrongLabelDetailStatistic(label=detail_key, amount=detail_value)
            )
        wrong_label_statistic = WrongLabelStatistic(label=key, detail=detail)

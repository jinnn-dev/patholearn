...
from app.schemas.course import CourseAdmin, CourseAll, CourseCreate, CourseDetail
...
def get_courses_of_user(
    ...
    for course in courses:
        ...
        correct_count = crud_user_solution.get_amount_of_correct_solutions_to_course(
...
def join_course(
    *,
    ...
    if joined_course is None:
        raise HTTPException(status_code=409, detail="Already part of the course")
    count = 0
...
def leave_course(
    *,
    ...
    return crud_user_solution.remove_all_by_user_to_course(
        db, user_id=current_user.id, course_id=course.id
    )
...
def create_course(
    *,
    ...
    if duplicate_course:
        raise HTTPException(
            status_code=400, detail="The Course with this name already exists"
        )
    course = crud_course.create_with_owner(
        db=db, obj_in=course_in, owner_id=current_user.id
    )
    return course

...
class CRUDSlide(CRUDBase[Slide, CreateSlide, UpdateSlide]):
    ...
    def get_all_slides(self, *, collection: Collection, status: SlideStatus = None, with_metadata: bool = True) -> List[Slide]:
        filter_query = {'_id': False, 'metadata': with_metadata}
        where_query = {}
        if status != None:
            where_query['status'] = status
        return self.get_multi(collection, where_query=where_query, filter_query=filter_query)

    def get_all_slides_by_ids(self, *, collection: Collection, slide_ids: List[str], status: SlideStatus = None, with_metadata: bool = True) -> \
            List[Slide]:
        filter_query = {'_id': False, 'metadata': with_metadata}
        where_query = {}
        if status != None:
            where_query['status'] = status
        return self.get_multi_by_ids(collection, slide_ids, where_query=where_query, filter_query=filter_query)

...
class CRUDBase(Generic[ModelSchemaType, CreateSchemaType, UpdateSchemaType]):
    ...
    def get_by_objectId(
        self, collection: Collection, *, object_id: ObjectId
    ) -> ModelSchemaType:
    ...
    def get(
        ...
        *,
        entity_id_value: Any,
        filter_qurey: Dict[str, Any] = None
    ) -> ModelSchemaType:
        return parse_obj_as(
            self.model,
            collection.find_one(
                {self.entity_id_name: entity_id_value},
                remove_truth_values_from_dict(filter_qurey),
            ),
        )
    ...
    def get_multi(
        ...
        return parse_obj_as(
            ...
            list(
                collection.find(
                    where_query, remove_truth_values_from_dict(filter_query)
                )
            ),
        )
    ...
    def get_multi_by_ids(
        ...
        return parse_obj_as(
            List[self.model],
            list(
                collection.find(find_query, remove_truth_values_from_dict(filter_query))
            ),
        )

    def create(
        self, collection: Collection, *, obj_in: CreateSchemaType, exclude_unset=True
    ) -> ObjectId:
    ...
    def update(
        self, collection: Collection, *, obj_in: UpdateSchemaType, entity_id_value: Any
    ) -> UpdateResult:
